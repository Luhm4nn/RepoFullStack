  ...
  try {
    ...
    for (const funcion of funcionesACrear) {
      try {
        ...
        if (existente) {
          logger.info(
            `⚠️  Ya existe: ${funcion.pelicula} en ${funcion.sala} - ${funcion.fechaHoraFuncion.toLocaleString('es-AR')}`
          );
          continue;
        }
        ...
        logger.info(
          `✅ Creada: ${funcion.pelicula} en ${funcion.sala} - ${funcion.fechaHoraFuncion.toLocaleString('es-AR')}`
        );
        creadas++;
      } catch (error) {
