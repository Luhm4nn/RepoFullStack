...
function MisReservasList({ reservas, onReservaActualizada }) {
  ...
  const getEstadoBadge = (estado, funcionPasada) => {
    ...
    if (
      funcionPasada &&
      (estado === ESTADOS_RESERVA.ACTIVA || estado === ESTADOS_RESERVA.ASISTIDA)
    ) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-400 mb-6">
          Explora nuestra cartelera y reserva tu película favorita
        </p>
        <a
          href="/cartelera"
    ...
    <div className="grid grid-cols-1 gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {getEstadoBadge(reserva.estado, funcionPasada) && (
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-semibold border ${getEstadoBadge(reserva.estado, funcionPasada)}`}
                          >
      ...
      {reservaAConfirmar && (
        <ModalCancelarReserva
          reserva={reservaAConfirmar}
          onConfirm={handleConfirmarCancelacion}
          onCancel={() => setReservaAConfirmar(null)}
          isCancelling={
            cancellingId ===
            `${reservaAConfirmar.idSala}-${reservaAConfirmar.fechaHoraFuncion}-${reservaAConfirmar.DNI}-${reservaAConfirmar.fechaHoraReserva}`
          }
        />
      )}
    </div>
