  ...
  const handleCrearReservaPendiente = async (values) => {
    ...
    try {
      ...
      const msg =
        ...
        'Error al bloquear los asientos. Posiblemente alguien los ocupó justo ahora.';
      setError(msg);
      notify.error(msg);
    } finally {
      setLoading(false);
                ...
                <Form className="max-w-md mx-auto py-10">
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 space-y-6 shadow-2xl">
                    <div>
                      <label
                        htmlFor="DNI"
                        className="block text-gray-400 text-xs font-black uppercase tracking-widest mb-3"
                      >
                        DNI para la reserva
                      </label>
                      <Field
                      ...
                      />
                      {errors.DNI && touched.DNI && (
                        <p className="text-red-500 text-xs mt-2 font-bold">{errors.DNI}</p>
                      )}
                    </div>
                    <button
                      type="submit"
                    ...
                    <button
                      type="button"
                      onClick={() => setStep(1)}
                      className="w-full text-gray-500 hover:text-gray-300 text-sm font-bold uppercase tracking-widest"
                    >
                      Atrás
                    </button>
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  <h3 className="text-xl font-black text-white uppercase flex items-center gap-3">
                    <span className="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center text-sm">
                      1
                    </span>
                    Resumen
                  </h3>
                  <div className="bg-slate-800/40 border-2 border-slate-700/50 rounded-2xl p-6 space-y-5">
                    ...
                    <div className="space-y-2">
                      <p className="text-gray-500 text-[10px] uppercase font-black">
                        Asientos Reservados
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {selectedSeatsData.seats.map((seat, idx) => (
                          <span
                            key={idx}
                            className="bg-purple-500/10 border border-purple-500/30 text-purple-400 px-3 py-1 rounded-lg font-black text-sm"
                          >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 text-red-500/70 hover:text-red-500 text-[10px] uppercase font-black transition-all"
                  >
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-800/40 border-2 border-slate-700/50 rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center text-center space-y-6">
                    <div className="w-16 h-16 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V7a2 2 0 00-2-2H4a2 2 0 00-2 2v10a2 2 0 002 2z"
                        />
                      </svg>
                    </div>
                    <div>
                      ...
                      <p className="text-gray-500 text-xs md:text-sm mt-1">
                        Haga clic abajo para finalizar su compra de forma segura.
                      </p>
                    </div>
                    <button
                      onClick={handleConfirmarReserva}
                      disabled={loading}
                      className="w-40 md:w-full mx-auto py-3 md:py-5 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-full font-black text-sm md:text-xl hover:scale-[1.02] shadow-2xl shadow-green-500/20 active:scale-95 transition-all disabled:opacity-50"
                    >
                      {loading ? <CenteredSpinner size="sm" /> : 'PAGAR AHORA'}
                    </button>
                    <p className="text-gray-400 text-[11px] md:text-sm mt-2">
                      ¿Deseas pagar en una pestaña aparte? Haz clic en el botón y sigue el flujo.
                    </p>
                  </div>
                </div>
              </div>
              {error && (
                <div className="mt-6 bg-red-500/10 border border-red-500/30 p-4 rounded-lg text-red-400 text-sm text-center font-bold animate-shake">
                  {error}
                </div>
              )}
            </div>
            ...
            <div className="py-10 text-center space-y-8 animate-fade-in">
              ...
              <div>
                ...
                <p className="text-gray-400 mt-2 font-medium">
                  Se han enviado los detalles a tu email.
                </p>
              </div>
              <div className="max-w-md mx-auto bg-slate-800 rounded-3xl p-8 border border-slate-700 shadow-2xl space-y-4">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 font-bold uppercase">Película</span>
                  <span className="text-white font-black">{pelicula.nombrePelicula}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 font-bold uppercase">Titular DNI</span>
                  <span className="text-white font-black">{confirmedDNI}</span>
                </div>
                <div className="flex justify-between text-base pt-4 border-t border-slate-700">
                  <span className="text-white font-black uppercase">Total Pagado</span>
              ...
              <button
                ...
                className="px-12 py-4 bg-white text-black rounded-2xl font-black text-lg hover:scale-105 transition-all active:scale-95"
              >
                FINALIZAR
              </button>
            </div>
