...
function ReservaPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        const [movieData, authData] = await Promise.all([
          getPelicula(id),
          authAPI.checkAuth()
        ]);
  ...
  const handleSelectFuncion = (funcion) => {
    if (!userDNI) {
      notify.warning("Inicia sesión para reservar tus entradas.");
  ...
  if (!pelicula) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-4">
        ...
        <button onClick={() => navigate("/")} className="text-purple-400 underline">Volver al inicio</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div id="funciones" className="bg-slate-800/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 shadow-2xl">
            ...
            <div className="animate-in fade-in duration-500">
              ...
              <div className="mt-12 flex flex-col items-center gap-6">
                 <div className="bg-white/5 border border-white/10 rounded-2xl px-8 py-4 w-full max-w-xs text-center shadow-inner">
                    <p className="text-gray-500 text-[10px] mb-1 uppercase tracking-[0.2em] font-bold">Total a pagar</p>
                    <span className="text-4xl font-black text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.3)]">${selectedSeatsInfo.total}</span>
                 </div>
                <button
