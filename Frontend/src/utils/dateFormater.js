...
export const formatDateTime = (dateTimeString) => {
  ...
  // CRITICO: Crear el objeto Date directamente desde el string ISO.
  // JavaScript lo interpreta automaticamente como UTC y lo convierte a la hora local.
  const dateObj = new Date(dateTimeString); 
  
  if (isNaN(dateObj.getTime())) {
    return { fecha: 'Fecha invalida', hora: 'Hora invalida' };
  }
  ...
  const hora = dateObj.toLocaleTimeString('es-LA', {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false // O true, segun prefieras
  });
...
export const formatDateTimeForBackend = (fechaHoraString) => {
  ...
  //    Esto corrige el problema de "3hs despues".
